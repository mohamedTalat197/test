...
<script type="text/javascript">
    ...
    function editFunction(id) {
        ...
        $.ajax({
            url: "{{get_baseUrl()}}/Admin/Category/edit/" + id,
            type: "GET",
            dataType: "JSON",
    ...
    function deleteFunction(id,type) {
        if (type == 2 && checkArray.length == 0) {
            ...
            url =  "{{get_baseUrl()}}/Admin/Category/destroy/" + id;
            deleteProccess(url);
        }else{
            url= "{{get_baseUrl()}}/Admin/Category/destroy/" + checkArray + '?type=2';
            deleteProccess(url);
            checkArray=[];
        }
    }
...
<script>
    function ChangeStatus(status,id) {
        ...
        $.ajax({
            url : '{{get_baseUrl()}}/Admin/Category/ChangeStatus/' +id +'?status='+status,
            type : 'get',
            success : function(data){
                $.toast().reset('all');
                table.ajax.reload();
...
<script>
    $('#seachForm').submit(function (e) {
        ...
        table.ajax.url('{{get_baseUrl()}}/Admin/Category/allData?' + formData);
        table.ajax.reload();
